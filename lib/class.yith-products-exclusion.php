	...
	class YWZM_Products_Exclusion {
		...
		public static function output() {
			...
			$table = new YITH_YWZM_Custom_Table( array(
				'singular' => __( 'product', 'yith-woocommerce-zoom-magnifier' ),
				'plural'   => __( 'products', 'yith-woocommerce-zoom-magnifier' )
			) );
			...
			$table->options = array(
				...
				'custom_columns'   => array(
					'column_product' => function ( $item, $me ) {
						...
						$actions = array(
							'delete' => '<a href="' . esc_url( $delete_url ) . '">' . __( 'Remove from exclusions', 'yith-woocommerce-zoom-magnifier' ) . '</a>'
						);

						return sprintf( '<strong><a class="tips" target="_blank" href="%s" data-tip="%s">#%d %s </a></strong> %s', esc_url( $product_url ), __( 'Edit product', 'yith-woocommerce-zoom-magnifier' ), $item['ID'], $item['post_title'], $me->row_actions( $actions ) );
					}
				),
				'bulk_actions'     => array(
					'actions'   => array(
						'delete' => __( 'Remove from list', 'yith-woocommerce-zoom-magnifier' )
					),
					'functions' => array(
			...
			if ( ! empty( $_POST['nonce'] ) && wp_verify_nonce( $_POST['nonce'], basename( __FILE__ ) ) ) {
				...
				if ( $item_valid !== true ) {
					...
					if ( ! empty( $_POST['insert'] ) ) {
						...
						$message = __( 'Product updated successfully', 'yith-woocommerce-zoom-magnifier' );
			...
			<div class="wrap">
				...
				<h2><?php _e( 'Single Product Exclusion List', 'yith-woocommerce-zoom-magnifier' );
					...
					if ( empty( $_GET['action'] ) || ( 'insert' !== $_GET['action'] ) ) : ?>
						...
						?>
						<a class="add-new-h2"
						   href="<?php echo esc_url( $add_form_url ); ?>"><?php _e( 'Add Products', 'yith-woocommerce-zoom-magnifier' ); ?></a>
					<?php endif; ?>
				</h2>
				<?php if ( ! empty( $notice ) ) : ?>
					...
					<form id="form" method="POST">
						...
						<table class="form-table">
							...
							<tr valign="top" class="titledesc">
								<th scope="row">
									<label for="product"><?php _e( 'Products to exclude', 'yith-woocommerce-zoom-magnifier' ); ?></label>
								</th>
								<td class="forminp yith-choosen">

									<?php if ( 'insert' == $_GET['action'] ) : ?>

										<select id="product" name="products[]" class="ajax_chosen_select_product"
										        style="width: 50%" multiple="multiple"
										        placeholder="Search for product"></select>

									<?php else : ?>

										<input id="product" name="products[]" type="hidden"
						...
						<?php if ( 'insert' == $_GET['action'] ) : ?>
							...
							<input type="submit" value="<?php _e( 'Add product exclusion', 'yith-woocommerce-zoom-magnifier' ); ?>" id="insert"
							       class="button-primary" name="insert">
						<?php endif; ?>
						<a class="button-secondary"
						   href="<?php echo esc_url( $list_url ); ?>"><?php _e( 'Return to exclusion list', 'yith-woocommerce-zoom-magnifier' ); ?></a>
					</form>
				<?php else : ?>
		...
		static function validate_fields( $item ) {
			...
			if ( empty( $item['products'] ) ) {
				$messages[] = __( 'Select at least one product', 'yith-woocommerce-zoom-magnifier' );
			}
